...
Object.keys(albums).forEach((item) => {
	...
	container.innerHTML = `<h4>${albums[item][0].title}</h4><p>${albums[item][0].releaseDate}</p>`
...
audio.addEventListener("playing", () => {
	...
	document.getElementById("play-pause").innerHTML = player.icons[1];
});
document.getElementById("play-pause").addEventListener("click", () => {
	if(!audio.paused) {
		audio.pause();
		document.getElementById("play-pause").innerHTML = player.icons[0];
...
function playAudio(album, format, track = 1) {
	...
	document.getElementById("musAlbum").innerHTML = albumName;
}
document.querySelectorAll(".music").forEach((item) => {
	item.addEventListener("click", () => {
		if(document.getElementById("musplayer").style.display == "block") player.isOpen = true;
		playAudio(item.id, albums[item.id][0].fileFormat);
	});
})
document.getElementById("go-back").addEventListener("click", () => {
	player.isOpen = true;
	if(musicInfo.track - 1 == 0) playAudio(musicInfo.album, musicInfo.lastTrack, musicInfo.format);
